...
/*
 *	Repr�senter les mol�cules par de belles sph�res �clair�es
 *	sous forme de texture plaqu�s sur un carr� (qui sera toujours
 *	affich� de face).
 */
...
#define	RADIUS (DIAMETER / 2)
	...
	glAlphaFunc (GL_GREATER, 0);	// pour masquer les bords des carr�s.
	...
	// presque la m�me, mais juste pour avoir un disque.
	for (i = 0; i < DIAMETER; i++) {
		for (j = 0; j < DIAMETER; j++) {
			int	dist = (RADIUS-i)*(RADIUS-i) + (RADIUS-j)*(RADIUS-j);
		...
		switch (button) {
			case GLUT_LEFT_BUTTON:
				...
				switch (mod) {
					...
					// s�lectionner le pivot de rotation.
					case 0:
						currentx = &site;
						currenty = &azimuth;
						return;
...
/*
 *	Dessine un coin sup�rieur gauche style Star Trek TNG.
 */
...
static void timer (int)
{
	...
	// fixer le facteur d'acc�l�ration en fonction du frame-rate.
	if (accel_std == 0.) {
		accel_std = 1.0;
...
/*
 *	Traite les �v�nements du clavier.
 */
...
static void keyboard (unsigned char key, int x, int y)
{
	switch (key) {
		...
		case '\b':		// r�tablit les conditions initiales.
			init_observer ();
			glutPostRedisplay ();
			break;
	}
